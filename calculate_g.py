...
integration_path = Path("/home/kdey/action-angle-discovery/integrations") / ("outer_planets" if outer_only else "full_system")
# %%
file_names = list(integration_path.glob("*.sa"))

transform_path = integration_path / "plots"
...
def Fourier_transform(filename):
	desn = filename.stem.split('_')[-1]
	row = merged_df[merged_df["Des'n"] == desn]

	results = cm.nbody_simulation_utilities.get_simarchive_integration_results(str(filename) ,coordinates='heliocentric')
	...
	inner_planet_offset = 0 if outer_only else 4
	...
	plt.savefig(transform_path / (filename.stem + ".png"))
	...
	g_list = [desn, row['g'].values[0]]
	for g in asteroid_e_freqs_arcsec_per_yr[:4]:
		g_list.append(g)
...
with Pool(20) as p:
	...
	df = pd.DataFrame(g_table, columns=["Des'n", 'g', 'g_1', 'g_2', 'g_3', 'g_4'])
	df.to_csv(integration_path / f"asteroid_g.csv", index=False)
# %%
